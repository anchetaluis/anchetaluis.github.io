...
var _observer3=new MutationObserver(function(){if(_this27.querySelector("template")){_observer3.disconnect();_this27.__render()}else{throw new Error("dom-if requires a <template> child")}});_observer3.observe(this,{childList:!0});return!1}this.__ctor=_templatize(template,this,{// dom-if templatizer instances require `mutable: true`, as
...
requestAnimationFrame(function(){self.fire("app-reset-layout")})},_notifyDescendantResize:function _notifyDescendantResize(){if(!this.isAttached){return}this._interestedResizables.forEach(function(resizable){if(this.resizerShouldNotify(resizable)){this._notifyDescendant(resizable)}},this)}}];_exports.AppLayoutBehavior=AppLayoutBehavior;var appLayoutBehavior={AppLayoutBehavior:AppLayoutBehavior};_exports.$appLayoutBehavior=appLayoutBehavior;Polymer({_template:html(_templateObject_cdfc8d10ff3911e9acc00ff41e4cebf6()),is:"app-drawer-layout",behaviors:[AppLayoutBehavior],properties:{/**
...
this.debounce("_resetDrawerState",this._resetDrawerState,this.transitionDuration)},_MIN_FLING_THRESHOLD:.2,_MIN_TRANSITION_VELOCITY:1.2,_FLING_TIMING_FUNCTION:"cubic-bezier(0.667, 1, 0.667, 1)",_FLING_INITIAL_SLOPE:1.5,_DRAWER_STATE:{INIT:0,OPENED:1,OPENED_PERSISTENT:2,CLOSED:3,TRACKING:4,FLINGING:5/**
                 ...
                 */ /**
                     ...
                     */}});Polymer({_template:html(_templateObject3_cdfc8d10ff3911e9acc00ff41e4cebf6()),is:"app-header-layout",behaviors:[AppLayoutBehavior],properties:{/**
...
if(30<Math.abs(this._initScrollTop-scrollTop)||10<absDScrollTop){if(isScrollingDown&&scrollTop>=maxHeaderTop){top=maxHeaderTop}else if(!isScrollingDown&&scrollTop>=this._dHeight){top=this.condenses&&!this.fixed?this._dHeight:0}var scrollVelocity=dScrollTop/(now-this._lastTimestamp);this.style.transitionDuration=this._clamp((top-lastTop)/scrollVelocity,0,300)+"ms"}else{top=this._top}}}if(0===this._dHeight){progress=0<scrollTop?1:0}else{progress=top/this._dHeight}if(!forceUpdate){this._lastScrollTop=scrollTop;this._top=top;this._wasScrollingDown=isScrollingDown;this._lastTimestamp=now}if(forceUpdate||progress!==this._progress||lastTop!==top||0===scrollTop){this._progress=progress;this._runEffects(progress,top);this._transformHeader(top)}},/**
   ...
   * @this Polymer.AppLayout.ElementWithBackground
   */setUp:function setUp(config){var title=this._getDOMRef("mainTitle"),condensedTitle=this._getDOMRef("condensedTitle"),duration=config.duration||"0.2s",fx={};if(!condensedTitle){console.warn("Scroll effect `resize-snapped-title`: undefined `condensed-title`");return!1}if(!title){console.warn("Scroll effect `resize-snapped-title`: undefined `main-title`");return!1}title.style.transitionProperty="opacity";title.style.transitionDuration=duration;condensedTitle.style.transitionProperty="opacity";condensedTitle.style.transitionDuration=duration;fx.condensedTitle=condensedTitle;fx.title=title;this._fxResizeSnappedTitle=fx},/** @this Polymer.AppLayout.ElementWithBackground */run:function run(p,y){var fx=this._fxResizeSnappedTitle;if(0<p){fx.title.style.opacity=0;fx.condensedTitle.style.opacity=1}else{fx.title.style.opacity=1;fx.condensedTitle.style.opacity=0}},/** @this Polymer.AppLayout.ElementWithBackground */tearDown:function tearDown(){var fx=this._fxResizeSnappedTitle;fx.title.style.transition="";fx.condensedTitle.style.transition="";delete this._fxResizeSnappedTitle}});;Polymer({_template:html(_templateObject5_cdfc8d10ff3911e9acc00ff41e4cebf6()),is:"app-toolbar"});var workingURL$1,urlDoc,urlBase,anchor;/**
                              * @param {string} path
                              * @param {string=} base
...
if(event.defaultPrevented){return}}}},_triggerKeyHandler:function _triggerKeyHandler(keyCombo,handlerName,keyboardEvent){var detail=Object.create(keyCombo);detail.keyboardEvent=keyboardEvent;var event=new CustomEvent(keyCombo.event,{detail:detail,cancelable:!0});this[handlerName].call(this,event);if(event.defaultPrevented){keyboardEvent.preventDefault()}}};_exports.IronA11yKeysBehavior=IronA11yKeysBehavior;var ironA11yKeysBehavior={IronA11yKeysBehavior:IronA11yKeysBehavior};_exports.$ironA11yKeysBehavior=ironA11yKeysBehavior;var IronControlState={properties:{/**
   ...
   * @return {void}
   */ready:function ready(){this.addEventListener("focus",this._boundFocusBlurHandler,!0);this.addEventListener("blur",this._boundFocusBlurHandler,!0)},_focusBlurHandler:function _focusBlurHandler(event){// In Polymer 2.0, the library takes care of retargeting events.
...
if(event.target===this){this._setFocused("focus"===event.type)}else if(this.__handleEventRetargeting){var target=/** @type {Node} */dom(event).localTarget;if(!this.isLightDescendant(target)){this.fire(event.type,{sourceEvent:event},{node:this,bubbles:event.bubbles,cancelable:event.cancelable})}}},_disabledChanged:function _disabledChanged(disabled,old){this.setAttribute("aria-disabled",disabled?"true":"false");this.style.pointerEvents=disabled?"none":"";if(disabled){// Read the `tabindex` attribute instead of the `tabIndex` property.
...
_exports.IronMeta=IronMeta;IronMeta[" "]=function(){};IronMeta.types={};IronMeta.prototype={get value(){var type=this.type,key=this.key;if(type&&key){return IronMeta.types[type]&&IronMeta.types[type][key]}},set value(value){var type=this.type,key=this.key;if(type&&key){type=IronMeta.types[type]=IronMeta.types[type]||{};if(null==value){delete type[key]}else{type[key]=value}}},get list(){var type=this.type;if(type){var items=IronMeta.types[this.type];if(!items){return[]}return Object.keys(items).map(function(key){return metaDatas[this.type][key]},this)}},byKey:function byKey(key){this.key=key;return this.value}};var metaDatas=IronMeta.types;Polymer({is:"iron-meta",properties:{/**
   ...
   * @return {*}
   */byKey:function byKey(key){return new IronMeta({type:this.type,key:key}).value}});var ironMeta={IronMeta:IronMeta};_exports.$ironMeta=ironMeta;Polymer({_template:html(_templateObject7_cdfc8d10ff3911e9acc00ff41e4cebf6()),is:"iron-icon",properties:{/**
...
switch(page){case"app-home":new Promise(function(res,rej){return _require.default(["./app-home.js"],res,rej)}).then(function(bundle){return bundle&&bundle.$appHome||{}});break;case"app-credits":new Promise(function(res,rej){return _require.default(["./app-credits.js"],res,rej)}).then(function(bundle){return bundle&&bundle.$appCredits||{}});break;case"app-404":new Promise(function(res,rej){return _require.default(["./app-404.js"],res,rej)}).then(function(bundle){return bundle&&bundle.$app$404||{}});break;}}}],[{key:"template",get:function get(){return html(_templateObject10_cdfc8d10ff3911e9acc00ff41e4cebf6())}},{key:"properties",get:function get(){return{page:{type:String,reflectToAttribute:!0,observer:"_pageChanged"},routeData:Object,subroute:Object}}},{key:"observers",get:function get(){return["_routePageChanged(routeData.page)"]}}]);return MyApp}(PolymerElement);window.customElements.define("my-app",MyApp);var $_documentContainer$5=document.createElement("template");$_documentContainer$5.innerHTML="<dom-module id=\"shared-styles\">\n\t<template>\n\t\t<style>\n\t\t\t.main {\n                padding: 1rem 2rem;\n                color: #fff;\n                background: rgba(0,0,0,.3);\n                height: fit-content;\n                width: 50%;\n                margin: 2rem auto;\n                border-radius: 5px;\n\t\t\t}\n\n\t\t\t.social-media,\n\t\t\th1,\n\t\t\th2,\n\t\t\th3 {\n\t\t\t\tfont-weight: 100;\n\t\t\t\tfont-style: normal;\n\t\t\t\ttext-align: center;\n\t\t\t}\n\n\t\t\ta,\n\t\t\ta:visited {\n\t\t\t\tcolor: #fff;\n\t\t\t}\n\n\t\t\th1 {\n\t\t\t\tcolor: #fff;\n\t\t\t\tfont-size: 3rem;\n\t\t\t\tmargin: 6rem auto auto;\n\t\t\t}\n\n\t\t\th3 {\n\t\t\t\tmargin-top: 3rem;\n\t\t\t\ttext-align: unset;\n\t\t\t}\n\t\t\t\n\t\t\tli {\n\t\t\t    padding: .5rem 0;\n\t\t\t}\n\t\t\t\n\t\t\t@media screen and (max-width: 780px) {\n                .main {\n                    margin: 2rem;\n                    padding: 16px 0;\n                    color: #fff;\n                    background: unset;\n                    height: initial;\n                    width: unset;\n                    border-radius: unset;\n                }\n                ul {\n                    padding: 0;\n                }\n                \n                li {\n                    display: flex;\n                    flex-flow: column;\n                    align-items: center;\n                    text-align: center;\n                }\n\t\t\t}\n\t\t</style>\n\t</template>\n</dom-module>";document.head.appendChild($_documentContainer$5.content)});