...
function runEffectsForProperty(inst, effects, dedupeId, prop, props, oldProps, hasPaths, extraArgs) {
  var ran = false;
  var rootProperty = hasPaths ? root$0(prop) : prop;
...
function notifyPath(inst, path, props) {
  var rootProperty = root$0(path);

  if (rootProperty !== path) {
    var eventName = camelToDashCase$0(rootProperty) + '-changed';
...
function runNotifyEffect(inst, property, props, oldProps, info, hasPaths) {
  var rootProperty = hasPaths ? root$0(property) : property;
  var path = rootProperty != property ? property : null;
  var value = path ? get$0(inst, path) : inst.__data[property];
...
function parseArg(rawArg) {
  ...
  if (!a.literal) {
    a.rootProperty = root$0(arg); // detect structured path (has dots)
...
function marshalArgs(data, args, path, props) {
  ...
  for (var i = 0, l = args.length; i < l; i++) {
    ...
    if (arg.literal) {
      ...
      if (arg.structured) {
        v = get$0(data, name); // when data is not stored e.g. `splices`
  ...
  function (_propertyEffectsBase) {
    ...
    babelHelpers.createClass(PropertyEffects, [{
      ...
      value: function _setPendingPropertyOrPath(path, value, shouldNotify, isPathNotification) {
        if (isPathNotification || root$0(Array.isArray(path) ? path[0] : path) !== path) {
          ...
          if (!isPathNotification) {
            var old = get$0(this, path);
            path =
            /** @type {string} */
            set$0(this, path, value); // Use property-accessor's simpler dirty check
      ...
      value: function _setPendingProperty(property, value, shouldNotify) {
        var isPath = this.__dataHasPaths && isPath$0(property);
      ...
      */
      ...
      value: function splice(path, start, deleteCount) {
        ...
        var array =
        /** @type {Array} */
        get$0(this, path, info); // Normalize fancy native splice handling of crazy start values
