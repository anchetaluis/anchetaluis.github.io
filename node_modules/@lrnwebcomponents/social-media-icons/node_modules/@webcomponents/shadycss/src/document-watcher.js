define(["exports", "./style-settings.js", "./style-transformer.js", "./style-util.js"], function (_exports, _styleSettings, _styleTransformer, _styleUtil) {
  /**
  @license
  ...
  */
  ...
  _exports.getCurrentScope = getCurrentScope;
  _exports.getOwnerScope = getOwnerScope;
  _exports.ensureCorrectScope = ensureCorrectScope;
  ...
  var scopeRegExp = new RegExp("".concat(_styleTransformer.default.SCOPE_NAME, "\\s*([^\\s]*)"));
  /**
   * @param {!Element} element
   * @return {string}
   */
  ...
  function getOwnerScope(node) {
    var ownerRoot = (0, _styleUtil.wrap)(node).getRootNode();
  ...
  function ensureCorrectScope(element) {
    ...
    var ownerRoot = (0, _styleUtil.wrap)(element).getRootNode();
    ...
    if (currentScope && ownerRoot === element.ownerDocument) {
      ...
      _styleTransformer.default.domRemoveScope(element, currentScope);
    } else if (babelHelpers.instanceof(ownerRoot, ShadowRoot)) {
      var ownerScope = getOwnerScope(element);

      if (ownerScope !== currentScope) {
        // node was scoped, but not by its current owner
        _styleTransformer.default.domReplaceScope(element, currentScope, ownerScope);
      }
    }
  }
  /**
  ...
  function ensureCorrectSubtreeScoping(element) {
    ...
    var unscopedNodes = window['ShadyDOM']['nativeMethods']['querySelectorAll'].call(element, ":not(.".concat(_styleTransformer.default.SCOPE_NAME, ")"));
    ...
    for (var j = 0; j < unscopedNodes.length; j++) {
      ...
      if (scopeForPreviouslyUnscopedNode) {
        _styleTransformer.default.element(unscopedNode, scopeForPreviouslyUnscopedNode);
      }
    }
  }
  /**
    ...
    for (var x = 0; x < mxns.length; x++) {
      ...
      for (var i = 0; i < mxn.addedNodes.length; i++) {
        ...
        if (currentScope && root === n.ownerDocument && !isElementWithBuiltCss(n)) {
          _styleTransformer.default.domRemoveScope(n, currentScope);
        } else if (babelHelpers.instanceof(root, ShadowRoot)) {
          var newScope = getOwnerScope(n); // rescope current node and subtree if necessary

          if (newScope !== currentScope) {
            _styleTransformer.default.domReplaceScope(n, currentScope, newScope);
  ...
  if (!_styleSettings.nativeShadow && !(window['ShadyDOM'] && window['ShadyDOM']['handlesDynamicScoping'])) {
    ...
    _exports.flush = flush = function flush() {
